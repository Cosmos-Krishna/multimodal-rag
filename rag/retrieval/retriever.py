...
from rag.embedding.embedder import EmbeddingConfig, _embed_texts
from rag.indexing.faiss_index import IndexedChunkRef, search
...
_COMPOUND_TERM_PATTERNS = [
    (re.compile(r"\blong[\s\-]term\b"), "long-term"),
    (re.compile(r"\bshort[\s\-]term\b"), "short-term"),
]
# Normalizes spaced vs hyphenated variants of known polarity-bearing
# compound terms before tokenizing, so "long term" (query) and
# "long-term" (source text) count as the same token instead of splitting
# into disjoint {"long","term"} / {"long-term"} sets. Narrow and
# explicit on purpose - not a general compound-word merger.


def _normalize_compounds(text: str) -> str:
    for pattern, replacement in _COMPOUND_TERM_PATTERNS:
        text = pattern.sub(replacement, text)
    return text


def _tokenize(text: str) -> set[str]:
    text = _normalize_compounds(text.lower())
...
class RetrieverConfig:
    ...
    section_title_rerank_weight: float = 0.08
    # Separate, smaller bonus for query/section_title overlap - rewards
    # chunks whose section heading structurally mirrors the query (a
    # strong specificity signal MiniLM doesn't get credit for), while
    # staying well below lexical_rerank_weight. Titles are short, so a
    # full title-overlap match is easier to hit incidentally than a
    # full body-text match; kept conservative (~half of the body
    # weight) so it nudges close rankings rather than overriding cosine
    # similarity or body-text evidence. Not tuned to any single query -
...
def retrieve(
    ...
    if retriever_config.lexical_rerank_weight or retriever_config.section_title_rerank_weight:
        ...
        raw_results = sorted(
            ...
            key=lambda pair: (
                ...
                * _lexical_overlap(query_tokens, pair[0].chunk_text)
                + retriever_config.section_title_rerank_weight
                * _lexical_overlap(query_tokens, pair[0].section_title or "")
            ),
            reverse=True,
        )
